...
function PaperSubmission() {
  ...
  const onSubmit = async (data: PaperSubmissionValues) => {
    try {
      ...
      router.push(`/dashboard/conference/${conferenceId}/new-submission/authors/${response.submissionId}`);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6">
      <div className="w-full max-w-2xl">
        ...
        <form className="space-y-6 border rounded-lg p-6" onSubmit={handleSubmit(onSubmit)}>
              ...
              {normalizedAreas.map((primaryArea, index) => (
                <React.Fragment key={index}>
                  ...
                  <SelectItem
                    ...
                    disabled
                    className="font-semibold text-[#374151] bg-[#f3f4f6] cursor-not-allowed"
                  >
                    {primaryArea.title}
                  </SelectItem>
                  ...
                  {primaryArea.secondary.map((secondaryArea, i) => (
                    <SelectItem
                      key={i}
                      value={secondaryArea}
                      className="pl-6 text-[#64748b] hover:bg-[#f8fafc]"
                    >
                      {secondaryArea}
                    </SelectItem>
                  ))}
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-3">
            <UploadButton
              ...
              appearance={{
                button:
                  "bg-[#0f172a] hover:bg-[#000000] text-[#ffffff] px-4 py-2 rounded-md font-medium",
        ...
        <div className="flex justify-center">
          ...
          >
            {(isUploading || isSubmitting || isPending) ? "Submitting..." : "Submit Paper"}
          </Button>
        </div>
      </form>
      </div>
